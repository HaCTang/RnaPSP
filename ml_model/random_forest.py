...
recall_df.to_csv(os.path.join(output_dir, 'RF_recall_vs_acceptance_rate.csv'), index=False)
...
plt.ylabel('Recall')
plt.title('rf Recall vs Acceptance Rate')
plt.grid(True)
plt.savefig(os.path.join(output_dir, 'RF_recall_acceptance.png'))
plt.close()